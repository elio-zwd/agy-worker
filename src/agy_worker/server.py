...
TOOLS = {
    ...
    "agy_status": (McpStatusRequest, "等待或查询任务状态。已有 revision 时传 after_revision；wait_ms 省略默认 50000，可按任务预计耗时自主选择 50000～600000。MCP 会把总等待预算切成内部最多 25 秒的 long-poll 并合并普通 progress/unchanged；成员 AI 发出 leader_question 或 AGY 提前进入终态时立即返回。没有 after_revision 时只读取即时快照。若返回 leader_question，先用 agy_answer 回答匹配的 task_id/question_id，再继续 status；不要编造问题 ID。若总预算结束仍非终态，直接再次调用，不要在两次 agy_status 之间向用户发送等待说明。"),
...
async def _coalesced_status(client, model):
    """在一次 MCP 总等待预算内吞掉高频 progress revision，但成员提问必须立即交回领导。"""
...
def build_server(client):
    ...
    instructions=("AGY/agy 在支持任务中指 agy_worker MCP。已知 workspace_id 和 command_id 时直接 agy_worker；未知时仅调用一次紧凑 agy_capabilities，不要为定位 AGY 先跑 git/rg/chat/CLI 探测。"
                  ...
                  "queued/running 已知 revision 时用 after_revision；agy_status 的 wait_ms 省略默认 50000，可按预计耗时自主选择 50000～600000。较长 build/test 优先选择较长预算以减少模型轮询；AGY 终态会提前返回。单次 status 内部会用最多 25 秒分片合并普通 progress/unchanged；成员发出 leader_question 时会立即返回。"
                  "若 status 返回 leader_question，读取其中 question，使用同一 task_id/question_id 调用 agy_answer 给出领导判断，再继续 agy_status；不得猜测或改写 question_id。若仍非终态，立即再次调用，不要在轮询之间向用户发送等待说明。"
