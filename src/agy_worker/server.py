...
TOOLS = {
    "agy_capabilities": (CapabilitiesRequest, "先调用此只读 MCP 工具发现参数范围、workspace_id、同仓库 worktree 和已登记命令。在编译、测试、日志、浏览器、图片或 Android UI 等本 Worker 已支持任务里，用户所说的 AGY/agy 只指本机 agy_worker MCP，不指聊天、线程、agent 或 subagent。通常省略 limits 使用默认值。"),
    "agy_worker": (WorkerRequest, "用户要求‘让 AGY/agy’执行编译、测试、日志、浏览器、图片或 Android UI 等已支持任务时使用此 MCP 工具；例如‘让 AGY 跑一下编译’。AGY 在这里不是聊天、线程、agent 或 subagent：不得先列出/读取聊天或线程，不得向聊天/subagent 发消息或等待 thread。也不得通过 terminal/shell 直接运行 agy/agy.exe（包括 agy -p），正常任务不要先跑 agy --help；MCP 不可用时明确报告，不得静默回退。异步返回 task_id；按任务授权，源码写入默认禁止。编译只提取错误，不分析或修复。"),
...
def build_server(client):
    ...
    instructions=("路由绑定：在本 Worker 已支持任务里，用户所说的 AGY/agy 只指本机 agy_worker MCP，不指 Codex 聊天、线程、agent 或 subagent；例如‘让 AGY 跑一下编译’应直接走 agy_capabilities（必要时）→ agy_worker → agy_status。"
                  "不得先列出/读取聊天或线程，不得向聊天/subagent 发消息或等待 thread；也不得通过 shell/terminal 直接运行 agy/agy.exe/agy -p，正常任务不要先跑 agy --help。MCP 不可用时明确报告，不得静默回退。"
                  "workspace_id 是登记别名，不是路径；同仓库 Git worktree 使用 workspace_path。通常省略 limits 使用默认值。"
