...
TOOLS = {
    ...
    "agy_worker": (WorkerRequest, "提交 AGY 杂活任务。异步返回 task_id；按任务授权，源码写入默认禁止。编译只提取错误，不分析或修复。"),
    "agy_continue": (ContinueRequest, "续接已完成的 AGY 会话并启动新进程，必须给 expected_turn 与完整本轮权限。"),
    "agy_status": (StatusRequest, "查询任务状态与有界结构化结果，可等待最多 25 秒的状态变化。"),
    "agy_cancel": (CancelRequest, "取消当前任务及所属进程；重复取消不会影响其他任务。"),
